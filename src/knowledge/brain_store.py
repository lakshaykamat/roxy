...
def brain_item_from_row(row: sqlite3.Row) -> BrainItem:
    return BrainItem(
        id=row["id"], content=row["content"], title=row["title"], summary=row["summary"],
        item_type=row["item_type"], tags=json.loads(row["tags_json"]),
        ...
        capture_mode=row["capture_mode"], status=row["status"],
        due_at=parse_timestamp(row["due_at"]) if row["due_at"] else None,
        timezone=row["timezone"], recurrence_rule=row["recurrence_rule"],
        created_at=parse_timestamp(row["created_at"]), updated_at=parse_timestamp(row["updated_at"]),
        completed_at=parse_timestamp(row["completed_at"]) if row["completed_at"] else None,
    )


def save_item(content: str, title: str, summary: str, item_type: str, tags: list[str], source_type: str, capture_mode: str, *, capture_key: str | None = None, source_url: str | None = None, source_published_at: datetime | None = None, due_at: datetime | None = None, timezone_name: str | None = None, recurrence_rule: str | None = None) -> BrainItem:
    now = format_timestamp(utc_now())
    with _brain_database() as connection:
        cursor = connection.execute(
            "INSERT OR IGNORE INTO brain_items "
            "(content, title, summary, item_type, tags_json, source_type, source_url, source_published_at, capture_mode, capture_key, status, due_at, timezone, recurrence_rule, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?)",
            (content.strip(), title.strip(), summary.strip(), item_type, json.dumps(tags), source_type,
             source_url, format_timestamp(source_published_at) if source_published_at else None,
             capture_mode, capture_key, format_timestamp(due_at) if due_at else None,
             timezone_name, recurrence_rule, now, now),
        )
        row_id = cursor.lastrowid if cursor.rowcount else connection.execute(
